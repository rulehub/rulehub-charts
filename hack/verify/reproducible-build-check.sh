...
#
# Usage: bash hack/verify/reproducible-build-check.sh [--values values.yaml]
...
bash "$ROOT_DIR/hack/verify/verify-deterministic.sh" "$VALUES_FILE"
...
bash "$ROOT_DIR/hack/tools/aggregate-integrity.sh" || true
