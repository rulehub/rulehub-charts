...
for f in "${FILES[@]}"; do
  ...
  if [[ -n $id ]]; then
    ...
    if (( match_ok==0 )); then
      # Allow different top-level domain prefixes in filename vs id (e.g., betting- vs igaming.)
      # Compute id suffix after first dot and compare normalized hyphenated forms as suffix
      id_suffix=${id#*.}
      suf_hy=${id_suffix//./-}; suf_hy=${suf_hy//_/-}
      base_n=$(echo "$base" | sed -E 's/\.yaml$//' | tr '_' '-')
      if [[ $base_n == "$suf_hy"* || $base_n == *"-$suf_hy"* ]]; then
        match_ok=1
      fi
    fi
    if (( match_ok==0 )); then
