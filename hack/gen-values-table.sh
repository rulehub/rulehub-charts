...
echo "# Values Reference Table (Generated)" > "$OUT_FILE"
echo >> "$OUT_FILE"
echo '> Regenerate via `make values-table`. Descriptions need schema enrichment.' >> "$OUT_FILE"
echo >> "$OUT_FILE"
echo '| Key | Type | Default | Description |' >> "$OUT_FILE"
echo '|-----|------|---------|-------------|' >> "$OUT_FILE"
...
printf '| %s | %s | %s | %s |\n' 'gatekeeper.enabled' 'boolean' "${gatekeeper_enabled_default:-true}" 'Enable Gatekeeper engine' >> "$OUT_FILE"
printf '| %s | %s | %s | %s |\n' 'kyverno.enabled' 'boolean' "${kyverno_enabled_default:-true}" 'Enable Kyverno engine' >> "$OUT_FILE"
printf '| %s | %s | %s | %s |\n' 'kyverno.validationFailureAction' 'string' "${kyverno_vfa_default:-}" 'Global Kyverno override ("", audit, enforce)' >> "$OUT_FILE"
...
if command -v yq >/dev/null 2>&1; then
  ...
  yq '.gatekeeper.policies | keys | .[]' values.yaml | while read -r key; do
    ...
    echo "| gatekeeper.policies.${key}.enabled | boolean | true | - |" >> "$OUT_FILE"
  done
  # Kyverno policies
  yq '.kyverno.policies | keys | .[]' values.yaml | while read -r key; do
    echo "| kyverno.policies.${key}.enabled | boolean | true | - |" >> "$OUT_FILE"
  done
else
...
echo >> "$OUT_FILE"
echo "*Generated automatically - do not edit manually*" >> "$OUT_FILE"
