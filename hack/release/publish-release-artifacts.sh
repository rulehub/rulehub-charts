...
#   --provenance: generate slsa-provenance.json (hack/gen/gen-provenance.sh) and, if --attest also set, attach with cosign --type slsaprovenance
...
if (( DO_PROVENANCE==1 )); then
  ...
  bash hack/gen/gen-provenance.sh --version "$VERSION" --package --out slsa-provenance.json || echo "[release] provenance generation failed" >&2
fi
