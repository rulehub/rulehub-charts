...
for file in "$TEMPLATES_DIR"/*.yaml; do
  ...
  if grep -q '^kind: *ConstraintTemplate' "$file"; then
    ...
    extracted_kind=$(awk 'BEGIN{kind="";inspec=0;incrd=0;incrdspec=0;innames=0} /^spec:/ {inspec=1} inspec && /^  crd:/ {incrd=1} incrd && /^    spec:/ {incrdspec=1} incrdspec && /^      names:/ {innames=1} innames && /^[[:space:]]*kind:[[:space:]]*[^[:space:]]+/ { if(kind==""){ sub(/^[[:space:]]*kind:[[:space:]]*/,"",$0); kind=$0; print kind; exit } }' "$file" || true)
