...
drift-verify: ## Verify no drift vs core index.json (auto-discovery or CORE_INDEX=/path) (fails on drift)
	bash hack/verify/verify-drift-index.sh $(INDEX)
SHELL := /bin/bash
...
.PHONY: help lint docs test deps values-table pre-commit-install pre-commit-run render-verify policies-sync gen-disable-diff id-name-align
...
package-clean: ## Package chart excluding placeholder policy YAML (removes *placeholder*.yaml)
	./hack/release/prepack-remove-placeholders.sh
...
supply-chain-pipeline: ## Run end-to-end supply chain pipeline (set ORG, CHART_VERSION) (package->push->sign->sbom->scan->attest)
	@if [ -z "$(CHART_VERSION)" ] || [ -z "$(ORG)" ]; then echo 'Set CHART_VERSION and ORG'; exit 2; fi; \
	bash hack/release/pipeline-supply-chain.sh --org $(ORG) --version $(CHART_VERSION) --sign --sbom --scan --attest-sbom --attest-vuln || exit $$?

vulns-verify: ## Vulnerability gate: fail if CRITICAL vulns found (ORG, CHART_VERSION) (args: SEVERITY=CRITICAL ALLOW=0)
	@if [ -z "$(CHART_VERSION)" ] || [ -z "$(ORG)" ]; then echo 'Set CHART_VERSION and ORG'; exit 2; fi; \
	bash hack/verify/verify-vulns.sh --org $(ORG) --version $(CHART_VERSION) --severity $(if [ -n "$(SEVERITY)" ]; then echo $(SEVERITY); else echo CRITICAL; fi) --allow $(if [ -n "$(ALLOW)" ]; then echo $(ALLOW); else echo 0; fi)

provenance: ## Generate SLSA provenance (CHART_VERSION required) -> slsa-provenance.json
	@if [ -z "$(CHART_VERSION)" ]; then echo 'Set CHART_VERSION'; exit 2; fi; \
	bash hack/gen/gen-provenance.sh --version $(CHART_VERSION) --package --out slsa-provenance.json
...
id-annotations: ## Verify every policy YAML has rulehub.id annotation
	./hack/verify/verify-id-annotation.sh

enforce-rationale: ## Verify enforced Kyverno policies have rationale comments in values.yaml
	./hack/verify/verify-enforce-rationale.sh

integrity-verify: ## Verify policy YAML integrity against manifest.json
	./hack/verify/verify-integrity.sh

gen-manifest: ## Generate manifest.json (inventory + hashes) and aggregate integrity hash
	bash hack/gen/gen-manifest.sh --strict
	# Optional schema verification (non-fatal if schema tool missing)
	@if command -v jq >/dev/null 2>&1; then bash hack/verify/verify-manifest-schema.sh || true; fi

manifest-verify: ## Validate manifest.json against schema and semantics
	./hack/verify/verify-manifest-schema.sh || true
...
manifest-diff: ## Compare two manifest.json files (OLD=prev/manifest.json NEW=manifest.json) (Added/Removed/Modified)
	...
	bash hack/tools/compare-manifest.sh --old "$(OLD)" --new "$$NEW_MANIFEST"

aggregate-integrity: ## Compute aggregate integrity sha256 across all policy YAML files
	bash hack/tools/aggregate-integrity.sh

labels-verify: ## Verify every rendered resource has chart/version label
	./hack/verify/verify-chart-version-label.sh

gen-policies: ## Generate (or partially regenerate) policy YAMLs from core sources (env: CORE_METADATA_DIR, CORE_TEMPLATES_DIR, optional: PARTIAL=id1,id2 MANIFEST=1 DEBUG=1)
	@if [ -z "$(CORE_METADATA_DIR)" ] || [ -z "$(CORE_TEMPLATES_DIR)" ]; then \
		echo 'Usage: make gen-policies CORE_METADATA_DIR=../core/metadata CORE_TEMPLATES_DIR=../core/templates/kyverno [PARTIAL=id1,id2] [MANIFEST=1] [DEBUG=1]'; exit 2; \
	fi; \
	CORE_METADATA_DIR=$(CORE_METADATA_DIR) \
	CORE_TEMPLATES_DIR=$(CORE_TEMPLATES_DIR) \
	./hack/gen/gen-policies.sh $(if $(PARTIAL),--partial $(PARTIAL),) $(if $(MANIFEST),--manifest,) $(if $(DEBUG),--debug,)

gen-gatekeeper-core: ## Generate Gatekeeper core templates+constraints (IDS="id1 id2" or --all)
	@if [ -n "$(IDS)" ]; then \
		./hack/gen/gen-gatekeeper-core.sh $(IDS); \
	else \
		./hack/gen/gen-gatekeeper-core.sh --all; \
	fi

gen-kyverno-core: ## Generate Kyverno core ClusterPolicies (IDS="id1 id2" or --all)
	@if [ -n "$(IDS)" ]; then \
		./hack/gen/gen-kyverno-core.sh $(IDS); \
	else \
		./hack/gen/gen-kyverno-core.sh --all; \
	fi

gen-gatekeeper-igaming: ## Generate Gatekeeper constraints for igaming IDs (IDS="id1 id2" or --all)
	@if [ -n "$(IDS)" ]; then \
		./hack/gen/gen-gatekeeper-igaming.sh $(IDS); \
	else \
...
schema-compare: ## Compare values.yaml against values.schema.json (structure & presence)
	./hack/tools/compare-values-schema.sh

schema-coverage: ## Estimate coverage percentage of values.yaml keys present in values.schema.json
	bash hack/tools/schema-coverage.sh

gen-disable-diff: ## Generate unified diff to disable listed policies (usage: make gen-disable-diff KEYS="k1 k2")
	@if [[ -z "$(KEYS)" ]]; then echo "Specify KEYS=\"policy1 policy2\"" >&2; exit 1; fi; \
	./hack/gen/gen-disable-diff.sh $(KEYS)

semver-suggest: ## Suggest next SemVer: make semver-suggest FILE=changes.txt or STRING="Added: ..."
	@if [[ -n "$(FILE)" ]]; then \
	  ./hack/tools/suggest-semver-bump.sh --file "$(FILE)"; \
	elif [[ -n "$(STRING)" ]]; then \
	  ./hack/tools/suggest-semver-bump.sh --string "$(STRING)"; \
	else \
...
semver-analyze: ## Analyze manifests diff for SemVer suggestion (OLD=prev/manifest.json NEW=manifest.json)
	...
	./hack/tools/semver-analyze.sh --old-manifest $(OLD) --new-manifest $(NEW)


gen-changelog: ## Generate / prepend CHANGELOG.md (VERSION=X.Y.Z OLD=prev/manifest.json NEW=manifest.json)
	@if [[ -z "$(VERSION)" ]] || [[ -z "$(OLD)" ]] || [[ -z "$(NEW)" ]]; then echo 'Usage: make gen-changelog VERSION=X.Y.Z OLD=prev/manifest.json NEW=manifest.json'; exit 2; fi; \
	./hack/gen/gen-changelog.sh --version $(VERSION) --old-manifest $(OLD) --new-manifest $(NEW)
...
pseudo-diff: ## Render current chart and previous release (PREV=chart-0.1.0.tgz) and show normalized diff
	...
	bash hack/tools/pseudo-diff-template.sh --prev "$(PREV)" --values "$${VALUES_FILE}" || exit $$?
...
gen-snapshots: ## Generate current snapshot documents (golden) into snapshots/current
	./hack/gen/gen-snapshots.sh values.yaml snapshots/current

gen-snapshot-hashes: ## Generate per-file sha256 hashes for snapshot directory (SNAP_DIR=snapshots/current)
	./hack/gen/gen-snapshot-hashes.sh $(if [ -n "$(SNAP_DIR)" ]; then echo $(SNAP_DIR); else echo snapshots/current; fi)

snapshots-compare: ## Compare baseline vs current generated snapshots (fail on diff)
	./hack/tools/compare-snapshots.sh snapshots/baseline values.yaml
...
template-diff: ## Diff current helm template vs git ref (REF=git-ref) (VALUES=values.yaml Y=1 CTX=5 NO_COLOR=1)
	...
	VALUES_FILE=${VALUES:-values.yaml}; \
	bash hack/tools/template-diff.sh --ref "$(REF)" --values "$$VALUES_FILE" $(if $(Y),--y,) $(if $(NO_COLOR),--no-color,) $(if $(CTX),--context $(CTX),)

verify-deterministic: ## Ensure two successive helm template runs are identical
	./hack/verify/verify-deterministic.sh
...
igaming-hygiene-verify: ## Verify igaming Gatekeeper templates hygiene (requiredLabel schema, msg prefix)
	bash hack/verify/verify-igaming-gk-hygiene.sh

dual-engine-verify: ## Verify every rulehub.id has both Kyverno and Gatekeeper entries in manifest.json
	./hack/verify/verify-dual-engine-coverage.sh $(if $(ALLOW),--allow,)

underscore-gate: ## Verify no new underscore policy key added without dash variant (BASE=HEAD~)
	./hack/verify/verify-underscore-gate.sh $(if $(BASE),--base $(BASE),)
...
orphan-policies: ## Detect orphan policy files (no key) and dangling keys (no file)
	./hack/tools/detect-orphan-policies.sh

gen-deprecated-stubs: ## Generate deprecated stub YAMLs for keys with deprecated_since (usage: make gen-deprecated-stubs [VALUES=values.yaml])
	./hack/gen/gen-deprecated-stubs.sh $(if $(VALUES),--values $(VALUES),)

schema-version-verify: ## Verify values.yaml schemaVersion matches values.schema.json const
	./hack/verify/verify-schema-version.sh

profile-matrix: ## Render matrix scenarios (minimal/full/gatekeeper-only/kyverno-only) and report resource counts
	./hack/test/test-profile-matrix.sh

profiles-verify: ## Verify profiles (activeProfiles) render declared policies (options: FORCE=1 QUIET=1)
	./hack/verify/verify-profiles.sh $(if $(VALUES),--values $(VALUES),) $(if $(FORCE),--force-matrix,) $(if $(QUIET),--quiet,)

profile-consistency: ## Verify internal consistency of profiles bundles (subset, duplicates, unknown keys)
	./hack/verify/verify-profile-consistency.sh $(if $(VALUES),--values $(VALUES),)
...
performance-verify: ## Verify helm template performance regression within threshold (PERF_THRESHOLD=0.20 RUNS=3) (update baseline: make performance-verify UPDATE=1)
	./hack/verify/verify-performance.sh $(if $(UPDATE),--update,) $(if $(PERF_THRESHOLD),--threshold $(PERF_THRESHOLD),) $(if $(RUNS),--runs $(RUNS),)

fuzz-policies: ## Fuzz random enable/disable of policies to detect template crashes (RUNS=20)
	./hack/test/fuzz-policies.sh $(if $(RUNS),--runs $(RUNS),)

gen-supported-k8s-versions: ## Regenerate Supported Kubernetes Versions section in README.md
	bash hack/gen/gen-supported-k8s-versions.sh

deprecation-verify: ## Verify deprecation window (OLD_VALUES=prev/values.yaml WINDOW=2)
	bash hack/verify/verify-deprecation-window.sh $(if $(OLD_VALUES),--old-values $(OLD_VALUES),) $(if $(WINDOW),--window $(WINDOW),)

soft-deletion-verify: ## Verify deprecated stub files are removed after soft deletion window (WINDOW=2)
	bash hack/verify/verify-soft-deletion.sh $(if $(WINDOW),--window $(WINDOW),)

stub-not-enforce-verify: ## Verify deprecated stub policies are not enforced
	bash hack/verify/verify-stub-not-enforce.sh $(if $(VALUES),--values $(VALUES),)
...
freeze-verify: ## Verify release freeze (REF=git-ref WINDOW of allowed changes) usage: make freeze-verify REF=v0.1.0 [ALLOW_CRITICAL=1 ALLOW_HACK=1]
	@if [[ -z "$(REF)" ]]; then echo 'Usage: make freeze-verify REF=<git-ref> [ALLOW_CRITICAL=1] [ALLOW_HACK=1]'; exit 2; fi; \
	 bash hack/verify/verify-freeze.sh --ref $(REF) $(if $(ALLOW_CRITICAL),--allow-critical,) $(if $(ALLOW_HACK),--allow-hack,)
...
removed-without-deprecation-verify: ## Verify removed policy files were previously deprecated (REF=previous-tag)
	@if [[ -z "$(REF)" ]]; then echo 'Usage: make removed-without-deprecation-verify REF=<prev-tag>'; exit 2; fi; \
	 bash hack/verify/verify-removed-without-deprecation.sh --ref $(REF)
...
release-publish: ## Package, tag (signed), push, sign, sbom, scan, provenance, attest (CHART_VERSION= X.Y.Z ORG=org) usage: make release-publish CHART_VERSION=0.1.0 ORG=myorg SIGN=1 SBOM=1 SCAN=1 PROVENANCE=1 ATTEST=1 PUSH_TAG=1
	@if [[ -z "$(CHART_VERSION)" ]] || [[ -z "$(ORG)" ]]; then echo 'Usage: make release-publish CHART_VERSION=X.Y.Z ORG=org [SIGN=1] [SBOM=1] [SCAN=1] [ATTEST=1] [PUSH_TAG=1]'; exit 2; fi; \
	 bash hack/release/publish-release-artifacts.sh --version $(CHART_VERSION) --org $(ORG) $(if $(SIGN),--sign,) $(if $(SBOM),--sbom,) $(if $(SCAN),--scan,) $(if $(PROVENANCE),--provenance,) $(if $(ATTEST),--attest,) $(if $(PUSH_TAG),--push-tag,)

build-annotations-verify: ## Verify build annotations present when configured (VALUES=values.yaml)
	./hack/verify/verify-build-annotations.sh $(if $(VALUES),$(VALUES),)

size-verify: ## Verify total rendered manifest size within threshold (THRESHOLD=256000, VALUES=values.yaml)
	./hack/verify/verify-size.sh $(if $(THRESHOLD),--threshold $(THRESHOLD),) $(if $(VALUES),--values $(VALUES),)

integrity-configmap-verify: ## Verify integrity ConfigMap exported when enabled (VALUES=values.yaml)
	./hack/verify/verify-integrity-configmap.sh $(if $(VALUES),--values $(VALUES),)

integrity-all: ## Generate manifest + aggregate hash, verify integrity & README badge
	$(MAKE) gen-manifest
	$(MAKE) aggregate-integrity
	$(MAKE) integrity-verify || true
	bash hack/verify/verify-integrity-badge.sh

dev-validate: ## Fast validate only changed YAML policy files (args: BASE=ref STAGED=1 ALL=1)
	./hack/tools/dev-validate-changed.sh $(if $(BASE),--base $(BASE),) $(if $(STAGED),--staged,) $(if $(ALL),--all,)

risk-test-plan: ## Generate prioritized policy test plan (FORMAT=markdown|text|json LIMIT_LOW=10)
	bash hack/tools/prioritize-risk-tests.sh $(if $(FORMAT),--format $(FORMAT),) $(if $(LIMIT_LOW),--limit-low $(LIMIT_LOW),)

kubeconform-verify: ## Verify kubeconform artifact checksum (OS=linux ARCH=amd64 VERSION=v0.7.0 INSTALL=1 to install)
	bash hack/verify/verify-kubeconform-checksum.sh $(if $(VERSION),--version $(VERSION),) $(if $(OS),--os $(OS),) $(if $(ARCH),--arch $(ARCH),) $(if $(INSTALL),--install,)
...
.PHONY: gen-plugin-index verify-plugin-index

gen-plugin-index: ## Generate dist/index.json for the Backstage plugin
	@bash hack/gen/gen-plugin-index.sh

verify-plugin-index: ## Verify dist/index.json is current and valid
	@bash hack/verify/verify-plugin-index.sh
...
plugin-index-sync: ## Export metadata from core and generate/verify charts plugin index (CORE_DIR=../rulehub)
	...
	RULEHUB_PLUGIN_METADATA_JSON="$$CORE_DIR/dist/plugin-index-metadata.json" bash hack/gen/gen-plugin-index.sh; \
	bash hack/verify/verify-plugin-index.sh
